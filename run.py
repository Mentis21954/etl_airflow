...
@task()
def extract_info_from_all_artists(list_artists_names: list):
    artist_contents = {}
    print(list_artists_names)
    print(type(list_artists_names))
    # extract for all artists' informations from last fm and store as a dict
    for name in list_artists_names:
...
with DAG(dag_id="ETL", start_date=pendulum.datetime(2021, 1, 1, tz="UTC"), schedule_interval=None,
    ...
    print('---------names out of task {} -------'.format(artists_names[:2]))
    content_df = extract_info_from_all_artists(artists_names[:2])
    # releases_df = extract_titles_from_artist(artists_names[0])
